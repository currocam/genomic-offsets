...
@njit
def _genomic_offset_helper(turnover, environmental_factors, target_environmental_factors):
    current_f, future_f = np.zeros(environmental_factors.shape), np.zeros(environmental_factors.shape)
...
class GradientForestGO:
    ...
    def __init__(self,
        ...
        try:
            ...
            print('install.packages("gradientForest", repos="https://R-Forge.R-project.org")')
    ...
    def fit(self,
        ...
        with np_cv_rules.context():
            y_r = ro.r.matrix(np.asarray(allele_frequency), nrow=allele_frequency.shape[0])
            x_r = ro.r.matrix(np.asarray(environmental_factors), nrow=environmental_factors.shape[0])
            ...
            self._gf = self._gradient_forest.gradientForest(
                ...
                response_vars=cols[environmental_factors.shape[1]:], ntree=self.n_trees
            )
            self.F = []
    ...
    def genomic_offset(self,
        ...
        diff = _genomic_offset_helper(self.F, environmental_factors, target_environmental_factors)
