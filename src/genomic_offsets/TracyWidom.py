"""
This code is adapted from  https://github.com/yymao/TracyWidom
...
def _find_u_v(x, f, dlnf, N, a, b, c):
    ...
    s = x**1.5
    xd2 = x * 2.0 * (dlnf - _dlnf_dx(x, N, a, b, c))
    vv = lambda uu: -s * ((xd2 + 3.0) * uu + xd2 * s) / (xd2 + 6.0)
    fu = lambda uu: f / _f(x, N, a, b, c, uu, vv(uu)) - 1.0
...
class TracyWidom(object):
    ...
    We use the tables in https://www.cl.cam.ac.uk/~aib29/TWinSplus.pdf
    and the asymptotic in https://arxiv.org/abs/1111.2761
    """
    ...
    def __init__(self, beta=2):
        ...
        dlnf = nd.Derivative(
            lambda xx: np.log(self.__cdf(-xx)), step=3.33e-3, n=1, order=7
        )(-x[1])
        ...
        dlnf = nd.Derivative(
            lambda xx: np.log(1.0 - self.__cdf(xx)), step=3.33e-3, n=1, order=7
        )(x[-2])
    ...
    def pdf(self, x):
        ...
        """
        xa = np.asanyarray(x)
        return nd.Derivative(self.cdf, step=0.08, n=1, order=5)(xa)
