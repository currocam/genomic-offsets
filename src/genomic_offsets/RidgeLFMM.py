...
def lfmm2(genotypes, environmental_factors, n_latent_factors, lambda_=1e-5):
    ...
    Bt = (
        np.linalg.inv(
            np.dot(environmental_factors.T, environmental_factors) + lambda_ * np.eye(p)
        )
        .dot(environmental_factors.T)
        .dot(genotypes - np.dot(U, Vt))
    )
    return U, Vt.T, Bt.T
...
class RidgeLFMM:
    ...
    def fit(
        ...
        environmental_factors = environmental_factors - np.mean(
            environmental_factors, axis=0
        )
        self.U, self.V, self.B = lfmm2(
            genotypes, environmental_factors, self.K, self.lambda_
        )

    def predict(
        self, target_environmental_factors: np.ndarray
    ):  # Environmental matrix (nxP)
        """
        ...
        """
        target_environmental_factors = target_environmental_factors - np.mean(
            target_environmental_factors, axis=0
        )
    ...
    def f_test(
        ...
        res_x = (
            ...
            - u @ np.linalg.lstsq(u, environmental_factors, rcond=None)[0]
        )
